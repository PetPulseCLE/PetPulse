...
void imu_data_task(void *pv) {
    ...
    xEventGroupWaitBits(imuEventGroup, IMU_EVT_MOTION_BIT, pdFALSE, pdTRUE, portMAX_DELAY);
    ...
    while (true) {
        ...
        if(imu.rpt.activity_classifier.get().mostLikelyState == BNO08xActivity::STILL) {
            if(esp_timer_get_time() - last_static_us > 300000000UL) {
            ...
            ESP_LOGI(TAG_DATA, "Static detected — waiting for motion");
            xEventGroupWaitBits(imuEventGroup, IMU_EVT_MOTION_BIT, pdFALSE, pdFALSE, portMAX_DELAY);
            imu_register_callbacks(current_mode);
        }
    }
}
...
void imu_motion_task(void *pv) {
    ...
    ESP_LOGI(TAG_MOTION, "Significant motion detector armed");

    while (true) {
        ESP_LOGI(TAG_MOTION, "Waiting for static to re-arm");
        xEventGroupWaitBits(imuEventGroup, IMU_EVT_STATIC_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
        ESP_LOGI(TAG_MOTION, "Static detected - CLEARING STATIC BIT AND RE-ARMING");

        imu.rpt.accelerometer.disable();
        imu.rpt.cal_gyro.disable();
        imu.rpt.cal_magnetometer.disable();
        imu.rpt.rv.disable();
        imu.rpt.step_counter.disable();
        imu.rpt.activity_classifier.disable();


        imu_rearm_sig_motion();
        imu.rpt.significant_motion.register_cb(sig_motion_cb);
        ESP_LOGI(TAG_MOTION, "Significant motion detector re-armed");
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
}
