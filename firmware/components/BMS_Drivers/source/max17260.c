...
static esp_err_t max17260_read_reg(uint8_t reg, uint16_t *val) {
    uint8_t data[2];
    esp_err_t ret = i2c_master_write_read_device(MAX17260_I2C_PORT, MAX17260_ADDRESS, &reg, 1, data, 2, pdMS_TO_TICKS(100));
    if (ret == ESP_OK) {
...
static esp_err_t max17260_write_reg(uint8_t reg, uint16_t val) {
    ...
    data[1] = val & 0xFF;
    data[2] = (val >> 8) & 0xFF;
    return i2c_master_write_to_device(MAX17260_I2C_PORT, MAX17260_ADDRESS, data, 3, pdMS_TO_TICKS(100));
}
