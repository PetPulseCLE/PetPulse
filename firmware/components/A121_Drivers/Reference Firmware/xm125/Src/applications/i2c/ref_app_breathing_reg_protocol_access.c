...
{
	*value = REF_APP_BREATHING_REG_APPLICATION_ID_ENUM_REF_APP_BREATHING;
}
...
{
	float float_value = i2c_ref_app_breathing_get_heart_rate();
...
{
	float float_value = i2c_ref_app_breathing_get_heart_rate_confidence();
...
{
	float float_value = i2c_ref_app_breathing_get_detected_distance();
...
void ref_app_breathing_reg_read_raw_data(uint32_t *value)
{
	uint16_t addr = acc_reg_protocol_get_current_address();
	uint16_t index = addr - REF_APP_BREATHING_REG_RAW_DATA_START_ADDRESS;
	*value = i2c_ref_app_breathing_get_raw_data_word(index);
}

void ref_app_breathing_reg_read_raw_data_length(uint32_t *value)
{
	*value = i2c_ref_app_breathing_get_raw_data_length();
}
