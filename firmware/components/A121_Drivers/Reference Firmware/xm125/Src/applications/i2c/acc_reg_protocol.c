...
extern void ref_app_breathing_reg_read_raw_data(uint32_t *value);
...
static const acc_reg_protocol_t *get_register_struct(uint16_t register_address)
{
	...
	// Special handling for Raw Data Range
	if (register_address >= REF_APP_BREATHING_REG_RAW_DATA_START_ADDRESS && 
		register_address < REF_APP_BREATHING_REG_RAW_DATA_END_ADDRESS)
	{
		static acc_reg_protocol_t raw_data_reg = { 0, ref_app_breathing_reg_read_raw_data, NULL };
		raw_data_reg.address = register_address;
		return &raw_data_reg;
	}
...
uint16_t acc_reg_protocol_get_current_address(void)
{
	return reg_address;
}
