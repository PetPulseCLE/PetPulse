...
esp_err_t power_manager_init(void) {
    // I2C bus is initialized by BSP via bsp_bus_init() — get the bus handle.
    i2c_master_bus_handle_t i2c_bus = bsp_get_i2c_bus_handle();
    ...
    while(retry > 0) {
        if (max17260_init(i2c_bus, &gauge_config) == ESP_OK) {
    ...
    while(retry > 0) {
        if (bq25896_init(i2c_bus) == ESP_OK) {
...
// USB input source detection state
static power_source_type_t detected_source = POWER_SOURCE_UNKNOWN;
static TickType_t source_detect_start_tick = 0;
static bool source_detect_pending = false;
...
// Classify input source and set IINLIM accordingly.
// Called from monitor task after the detection delay has elapsed.
static void monitor_classify_source(void) {
    bool is_usb_host = usb_serial_jtag_is_connected();
    uint16_t iinlim;

    if (is_usb_host) {
        detected_source = POWER_SOURCE_USB_HOST;
        iinlim = CHARGER_USB_IINLIM_MA;
        ESP_LOGI(TAG, "Input source: USB host — IINLIM %umA", iinlim);
    } else {
        detected_source = POWER_SOURCE_ADAPTER;
        iinlim = CHARGER_ADAPTER_IINLIM_MA;
        ESP_LOGI(TAG, "Input source: adapter — IINLIM %umA", iinlim);
    }

    if (pm_state.i2c_mutex != NULL &&
        xSemaphoreTake(pm_state.i2c_mutex, pdMS_TO_TICKS(BMS_MUTEX_TIMEOUT_MS)) == pdTRUE) {
        bq25896_set_input_current_limit(iinlim);
        I2C_GIVE();
    }

    power_event_t e = { .type = POWER_EVENT_SOURCE_DETECTED,
                        .input = { .source = detected_source,
                                   .iinlim_ma = iinlim } };
    fire_event(&e);
    source_detect_pending = false;
}

// Reset input source state to safe defaults (called on VBUS disconnect).
static void monitor_reset_source(void) {
    detected_source = POWER_SOURCE_UNKNOWN;
    source_detect_pending = false;
    source_detect_start_tick = 0;

    // Restore safe USB default in case next plug-in is a host
    if (pm_state.i2c_mutex != NULL &&
        xSemaphoreTake(pm_state.i2c_mutex, pdMS_TO_TICKS(BMS_MUTEX_TIMEOUT_MS)) == pdTRUE) {
        bq25896_set_input_current_limit(CHARGER_USB_IINLIM_MA);
        I2C_GIVE();
    }
}
...
static void monitor_poll_charger(void) {
    ...
    // Refresh the charger-derived fields of the cached snapshot *before*
    // firing transition events. Listeners that call
    // power_manager_get_snapshot() from their event handler otherwise see
    // stale state from the last 30s gauge poll — which is why the BLE BAS
    // charging-state update appeared intermittent on plug/unplug.
    bool now_usb = (status.VBUS_STAT != VBusStatNo && status.VBUS_STAT != VBusStatOTG);
    portENTER_CRITICAL(&snap_spinlock);
    if (snap_valid) {
        cached_snap.usb_connected = now_usb;
        cached_snap.charge_status = status.CHRG_STAT;
    }
    ...
    if (now_usb && !was_usb) {
        ...
        // Begin source detection — IINLIM is already at safe 500mA from init.
        // We wait BMS_SOURCE_DETECT_DELAY_MS for USB host enumeration before classifying.
        source_detect_pending = true;
        source_detect_start_tick = xTaskGetTickCount();
        ...
        // Reset to safe defaults for next plug-in
        monitor_reset_source();
    }
...
static void monitor_poll_gauge(void) {
    ...
    if (pm_state.charger_ok) {
        ...
        snap.iinlim_ma     = bq25896_get_input_current_limit();
    }
    snap.input_source = detected_source;
...
static void bms_monitor_task(void *arg) {
    ...
    if (pm_state.charger_ok && pm_state.i2c_mutex != NULL) {
        if (xSemaphoreTake(pm_state.i2c_mutex, pdMS_TO_TICKS(BMS_MUTEX_TIMEOUT_MS)) == pdTRUE) {
            ...
            // If USB is already present at boot, start source detection now.
            // IINLIM is already safe (500mA from init).
            bool usb_present = (st.VBUS_STAT != VBusStatNo && st.VBUS_STAT != VBusStatOTG);
            if (usb_present) {
                ESP_LOGI(TAG, "USB present at boot — starting source detection");
                source_detect_pending = true;
                source_detect_start_tick = xTaskGetTickCount();
            }
        }
    ...
    while (monitor_running) {
        // If source detection is pending, use a shorter wait so we can
        // check the timer without waiting for the full gauge poll interval.
        TickType_t wait_ticks = poll_ticks;
        if (source_detect_pending) {
            TickType_t elapsed = xTaskGetTickCount() - source_detect_start_tick;
            TickType_t needed = pdMS_TO_TICKS(BMS_SOURCE_DETECT_DELAY_MS);
            wait_ticks = (elapsed >= needed) ? 0 : (needed - elapsed);
        }
        ...
        // Source detection: classify after delay has elapsed
        if (source_detect_pending) {
            TickType_t elapsed = xTaskGetTickCount() - source_detect_start_tick;
            if (elapsed >= pdMS_TO_TICKS(BMS_SOURCE_DETECT_DELAY_MS)) {
                monitor_classify_source();
            }
        }
...
bool power_manager_gauge_is_ok(void) {
    ...
    if(max17260_get_voltage() == 0) {
        // Double check status register if voltage is 0 (unlikely for working battery)
        if (max17260_check_communication() != ESP_OK) {
            ret = false;
        }
    }
