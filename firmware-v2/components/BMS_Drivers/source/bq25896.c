...
static esp_err_t bq25896_read_reg(uint8_t reg_addr, uint8_t *data) {
    return i2c_master_transmit_receive(s_bq25896_dev, &reg_addr, 1, data, 1, 100);
}

// Helper function to write a register
static esp_err_t bq25896_write_reg(uint8_t reg_addr, uint8_t data) {
    uint8_t write_buf[2] = {reg_addr, data};
    return i2c_master_transmit(s_bq25896_dev, write_buf, 2, 100);
}

// Helper to read all registers into shadow struct
static esp_err_t bq25896_read_all_regs(void) {
    uint8_t reg_addr = 0x00;
    return i2c_master_transmit_receive(s_bq25896_dev, &reg_addr, 1, (uint8_t*)&bq25896_regs, sizeof(bq25896_regs), 100);
}
...
esp_err_t bq25896_init(i2c_master_bus_handle_t bus_handle) {
    ...
    if (s_bq25896_dev == NULL) {
        i2c_device_config_t dev_cfg = {
            .dev_addr_length = I2C_ADDR_BIT_LEN_7,
            .device_address  = BQ25896_ADDRESS,
            .scl_speed_hz    = BSP_I2C_FREQ_HZ,
        };
        esp_err_t err = i2c_master_bus_add_device(bus_handle, &dev_cfg, &s_bq25896_dev);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to add BQ25896 to I2C bus: %d", err);
            return err;
        }
    }
    esp_err_t ret;
    ...
    // CRITICAL SAFETY: Disable automatic input source detection.
    // PSEL is tied to GND on this board, so the chip always detects DCP adapter
    // and sets IINLIM to 3.25A — dangerous when connected to a USB host (max 500mA).
    // Disabling AUTO_DPDM_EN prevents the chip from overwriting our safe IINLIM.
    bq25896_regs.r02.AUTO_DPDM_EN = 0;
    ...
    // Set safe input current limit BEFORE anything else that could draw power.
    // 500mA is the USB 2.0 configured device limit — Default safe point for any source.
    // Will be modified by monitor_classify_source() later if we detect a higher-capability input source classified by the ESP.
    ret = bq25896_set_input_current_limit(BQ25896_USB_DEFAULT_IINLIM_MA);
    if (ret != ESP_OK) return ret;
...
uint16_t bq25896_get_input_current_limit(void) {
    bq25896_read_reg(0x00, (uint8_t*)&bq25896_regs.r00);
    return (uint16_t)bq25896_regs.r00.IINLIM * BQ25896_IINLIM_STEP_MA + BQ25896_IINLIM_BASE_MA;
}
