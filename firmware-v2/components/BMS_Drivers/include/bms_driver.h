...
// Input current limits per detected source type (mA)
// IINLIM caps total draw from the source; ICHG (above) independently caps battery charge rate.
#define CHARGER_USB_IINLIM_MA          500     // USB 2.0 host — spec-safe default
#define CHARGER_ADAPTER_IINLIM_MA      2000    // Wall adapter / power bank (no USB host detected)
...
/** Detected USB input source type */
typedef enum {
    POWER_SOURCE_UNKNOWN,               ///< Not yet determined
    POWER_SOURCE_USB_HOST,              ///< USB host detected (SOF packets via Serial/JTAG)
    POWER_SOURCE_ADAPTER,               ///< No USB host — wall adapter or power bank
} power_source_type_t;
...
typedef enum {
    ...
    POWER_EVENT_SOURCE_DETECTED,        ///< Input source classified (USB host vs adapter)
...
typedef struct {
    ...
    union {
        ...
        struct {
            power_source_type_t source; ///< Classified input source
            uint16_t iinlim_ma;         ///< Applied input current limit (mA)
        } input;
    };
} power_event_t;
...
typedef struct {
    ...
    power_source_type_t input_source; ///< Detected input source type
    uint16_t iinlim_ma;         ///< Current input current limit (mA)
...
#define BMS_SOURCE_DETECT_DELAY_MS  1500    ///< Wait for USB host enumeration before classifying source
