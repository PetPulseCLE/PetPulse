...
typedef struct bno08x_config_t
{
        ...
        /// Pin assignments are sourced from BSP (bsp.h)
        bno08x_config_t(bool install_isr_service = false)
            : spi_peripheral(BSP_SPI1_HOST)
            , io_mosi(BSP_SPI1_MOSI)
            , io_miso(BSP_SPI1_MISO)
            , io_sclk(BSP_SPI1_CLK)
            , io_cs(BSP_CS_IMU)
            , io_int(BSP_INT_IMU)
            , io_rst(BSP_IMU_RST)
            , sclk_speed(static_cast<uint32_t>(CONFIG_ESP32_BNO08X_SCL_SPEED_HZ))
            , install_isr_service(install_isr_service) // false: BSP installs ISR service
        {
        }
