...
void radar_config_set_preset(a121_vitals_config_t *config, vitals_preset_t preset)
{
	...
	switch (preset)
	{
		...
		case VITALS_PRESET_PET_HARNESS:
			/*
			 * On-body harness mounted ~2–10 cm from pet chest, through fabric and fur.
			 * Target: dogs (60–140 BPM rest, up to ~200 stressed) and cats (120–240 BPM).
			 * See TODO.md for parameter rationale.
			 */
			config->lowest_breathing_rate             = 10U;
			config->highest_breathing_rate            = 60U;
			config->lowest_heart_rate                 = 40U;   /* 0.67 Hz — deep sleep floor */
			config->highest_heart_rate                = 270U;  /* 4.5 Hz — covers cats + margin */
			config->time_series_length_s              = 20U;
			config->num_dists_to_analyze              = 3U;
			config->use_presence_processor            = true;
			config->distance_determination_duration_s = 3U;

			/* Range: only Profile 1 resolves <10 cm targets cleanly. */
			acc_detector_presence_config_start_set(pc, 0.02f);
			acc_detector_presence_config_end_set(pc, 0.10f);
			acc_detector_presence_config_auto_profile_set(pc, false);
			acc_detector_presence_config_profile_set(pc, ACC_CONFIG_PROFILE_1);
			acc_detector_presence_config_auto_step_length_set(pc, false);
			acc_detector_presence_config_step_length_set(pc, 2U);  /* ~5 mm bins */

			/* Framing: 20 Hz gives ~2.3 BPM HR FFT resolution and 20 phase samples/s. */
			acc_detector_presence_config_frame_rate_set(pc, 20.0f);
			acc_detector_presence_config_sweeps_per_frame_set(pc, 16U);
			acc_detector_presence_config_hwaas_set(pc, 32U);

			/* Detection/tracking tuned for on-body strong signal and faster pet dynamics. */
			acc_detector_presence_config_intra_detection_threshold_set(pc, 3.0f);
			acc_detector_presence_config_intra_output_time_const_set(pc, 0.3f);
			acc_detector_presence_config_inter_output_time_const_set(pc, 3.0f);
			acc_detector_presence_config_inter_frame_fast_cutoff_set(pc, 20.0f);
			acc_detector_presence_config_inter_phase_boost_set(pc, true);
			acc_detector_presence_config_inter_frame_presence_timeout_set(pc, 0U);

			/* Battery-optimised: A121 deep-sleep wake (<2 ms) fits the 50 ms inter-frame gap. */
			acc_detector_presence_config_inter_frame_idle_state_set(pc, ACC_CONFIG_IDLE_STATE_DEEP_SLEEP);
			break;
